            ...
            appointments.map((item,index)=>(
                <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                    ...
                    <div className='flex flex-col gap-2 justify-end'>
                       ...
                       {!item.cancelled && !item.payment && !item.isCompleted &&<button onClick={()=>appointmentRazorPay(item._id)} className='hover:bg-primary hover:text-white transition-all duration-300 text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded'>
                            Pay Online
                            </button>}
                            {!item.cancelled&& !item.isCompleted &&<button onClick={()=>cancelAppointment(item._id)} className='hover:bg-red-600 hover:text-white transition-all duration-300 text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded'>
                                Cancel Appointment
                            </button>}
